...
int CgSkins::SkinScan(const char *pName, int IsDir, int DirType, void *pUser)
{
	...
	char aBuf[512];
	str_format(aBuf, sizeof(aBuf), "textures/%s", pName);
	CImageInfo Info;
	if(!pSelf->Graphics()->LoadPNG(&Info, aBuf, DirType))
	{
		str_format(aBuf, sizeof(aBuf), "failed to load textures from %s", pName);
	...
	if(g_Config.m_Debug)
	{
		str_format(aBuf, sizeof(aBuf), "load textures %s", Skin.m_aName);
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_ADDINFO, "game", aBuf);
	}
...
void CgSkins::OnInit()
{
	...
	{
		str_format(aBuf, sizeof(aBuf), "failed to load default textures");
	...
	if(g_Config.m_Debug)
	{
		str_format(aBuf, sizeof(aBuf), "load default textures", DefaultSkin.m_aName);
		Console()->Print(IConsole::OUTPUT_LEVEL_ADDINFO, "game", aBuf);
	}
	m_aSkins.add(DefaultSkin);
	
	Storage()->ListDirectory(IStorage::TYPE_ALL, "textures", SkinScan, this);
		
	if(!m_aSkins.size())
	{
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "gameclient", "failed to load textures. folder='textures/'");
		CgSkin DummySkin;
